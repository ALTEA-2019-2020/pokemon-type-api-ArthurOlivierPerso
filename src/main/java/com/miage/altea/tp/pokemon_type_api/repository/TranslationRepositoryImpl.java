...
public class TranslationRepositoryImpl implements TranslationRepository {
    ...
    public String getPokemonName(int id, Locale locale) {
        if (translations.containsKey(locale)) {
            return translations.get(locale).stream().filter(translation -> translation.getId()==id).findFirst().orElseThrow().getName();
        }
        else{
            return defaultTranslations.stream().filter(translation -> translation.getId()==id).findFirst().orElseThrow().getName();
        }
    }
}